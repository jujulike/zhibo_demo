...
<h4 class="marginbot">
	内容管理 >> 地区级管理
		...
		<div class="mainbox">
			...
			<lable>地区三级管理</lable>&nbsp;<input type="text" name="name" id="name" />&nbsp;<a href="javascript:submitAdd()" class="sgbtn" >添加</a>
			</form>
            <div class="provincelist">
				...
				<dt style="width:210px">
					<span style="float:left"><a href="<?php echo site_url('admin/region/area/' .$item['regionid']);?>"><?php echo $item['name']?></a></span>
					<a href="javascript:modi(<?php echo $item['regionid']?>,'<?php echo $item['name'] ?>')" class="sgbtn" style="font-size:12px">修改</a><a href="javascript:delregion(<?php echo $item['regionid']?>)" class="sgbtn" style="font-size:12px">删除</a>
				</dt>
			<?php }?>
            </div>
        </div>		
...
function submitAdd()
{
	postdata('regionForm', "<?php echo site_url('admin/region/add/'.$parentid.'/2');?>", 'show');
}
