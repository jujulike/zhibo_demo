...
<div class="so_main">
  ...
  <div id="searchContent_div" style="display:none;">
	...
	<div class="form2">
	 <form id="form3" name="form3" action="<?php echo site_url("admin/chatcontent");?>" method="post">
    <dl class="lineD">
      <dt>内容搜索：</dt>
      <dd>
  ...
  <tr>
    ...
    <th class="line_l" width="5%">操作</th>
	  ...
	  <tr overstyle='on' id="user_1">
		...
		<td><?php echo str_replace('<img ', '<img  onLoad="DrawImage(this,200,100)"', $v['chatcontent']); ?></td>
		<td><?php echo date("m-d H:i:s", $v['ctime']); ?></td>
	    <td>
			<a href="javascript:delContent(<?php echo $v['chatid']; ?>);" class="sgbtn">删除</a>
					</td>
			</tr>
		<?php } }?>
		</form>
...
function delContent(chatid)
{
	var submit = function (v, h, f) {
		if (v == 'ok')
			$.get("<?php echo site_url('admin/chatcontent/del');?>"+"/"+chatid, 
			function(d){
				var data = eval('('+d+')');
				wshow(data);
...
function delmore()
{
	var submit = function (v, h, f) {
		if (v == 'ok')
			postdata('form1', "<?php echo site_url('admin/chatcontent/delmore');?>", 'wshow');
		else if (v == 'cancel')
			return true;
	}
